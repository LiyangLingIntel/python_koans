...
class AboutLists(Koan):
    ...
    def test_list_literals(self):
        ...
        nums[1] = 3
        self.assertListEqual([1, 3, 333], nums)

        nums[1:][0] = 999
        self.assertListEqual([1, 3, 333], nums)

        def copy_or_reference(sliced_list):
            sliced_list = [0, 0, 0]

        copy_or_reference(nums[1:])
        self.assertEqual([1, 3, 333], nums)

        def copy_or_reference2(sliced_list):
            sliced_list[0] = 999

        copy_or_reference2(nums[1:])
        self.assertEqual([1, 3, 333], nums)

        def copy_or_reference3(sliced_list):
            sliced_list[1:] = [0, 0, 0]

        copy_or_reference3(nums)
        self.assertEqual([1, 0, 0, 0], nums)

        # The key is the time when [...] __getitem()__ or __setitem__
        # __getitem__ return a copy of original list element or list slicing
        # __setitem__ return a reference to list start address
    ...
    def test_slicing_lists(self):
        ...
        self.assertEqual(['jelly', 'and', 'butter'], noms[5:0:-1])
    ...
    def test_insertions(self):
        ...
        knight.insert(2, 'not')
        self.assertEqual(['you', 'shall', 'not', 'pass'], knight)  # Gandalf!
