...
class AboutAttributeAccess(Koan):
    ...
    def test_calling_undefined_functions_normally_results_in_errors(self):
        ...
        # with object will call its cls.__enter()__,
        # after with body finishes, call cls.__exit()__
        # here in _AssertRaisesContext, it expects AttributeError when exits
    ...
    class RecursiveCatcher:
        ...
        def __getattribute__(self, attr_name):
            ...
            if stack_depth <= 10:  # to prevent a stack overflow
                ...
                # accessed?
                # Note: self.__getattribute__(attr_name) will be called.
    ...
    class MinimalCatcher:
        ...
        # If a class defines __getattr__, it will be called every time an attribute cannot
        # be found in objects' attribute dictionary.
        # __getattribute__ is for everytime any attribute being called
        # All above and below is Python Class Lazy Attribute feature
        def __getattr__(self, attr_name):
            self.no_of_getattr_calls += 1
            return self.DuffObject

        def my_method(self):
            print(self.no_of_getattr_calls)
    ...
    class PossessiveSetter(object):
        # __setattr__ will be positively called, when everytime attribute is assigned to an instance
        # especially in __init__, should be careful
