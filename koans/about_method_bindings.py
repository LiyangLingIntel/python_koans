...
class AboutMethodBindings(Koan):
    ...
    def test_methods_have_different_attributes(self):
        ...
        diff_instance_method = [i for i in dir(function) if i not in dir(obj.method)]
        diff_methods = ['__annotations__', '__closure__', '__code__', '__defaults__', '__dict__', '__globals__',
                        '__kwdefaults__', '__module__', '__name__', '__qualname__']
        self.assertListEqual(diff_methods, diff_instance_method)

        diff_common_method = [i for i in dir(obj.method) if i not in dir(function)]
        diff_methods = ['__func__', '__self__']
        self.assertListEqual(diff_methods, diff_common_method)
    ...
    def test_get_descriptor_resolves_attribute_binding(self):
        # return an instance will call it __get__ method
